...
import { postPath, type PostUrlParts } from "@/lib/post-url";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [posts, groups] = await Promise.all([
    ...
    supabase
      .from("posts")
      .select("short_id, slug, kind, updated_at, group:groups(visibility)")
      .eq("is_hidden", false)
  ...
  return [
    ...
    // The topic hubs. Seven fixed pages, each the landing page for a whole slice
    // of the site, so they sit above group and post pages here.
    ...TOPICS.map((topic) => ({
      url: `${siteUrl}${topicPath(topic)}`,
    ...
    ...(posts.data ?? [])
      ...
      .map((post) => ({
        url: `${siteUrl}${postPath(post as unknown as PostUrlParts)}`,
